  ...
  return (
    <>
        <Card className="w-1/2 mx-auto mt-10">
            ...
            <CardFooter>
            <Button onClick={() =>{
                    cart.find((item: any) => item._id === product._id) ? handleRemoveFromCart(product?._id) : handleAddToCart(product)
                }}>
                {
                    cart.find((item: any) => item._id === product._id) ? "Remove from cart" : "Add to cart"
                }
            </Button>
            </CardFooter>
        </Card>
